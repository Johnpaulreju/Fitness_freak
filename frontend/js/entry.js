...
document.addEventListener('DOMContentLoaded', async function() {
        ...
        if (response.ok && data) {
            ...
            if (data.targetBodyType && document.getElementById('dietPlan')) {
                ...
                if (window.weightChart) window.weightChart.destroy();
                            ...
                            datasets: [
                                {
                                    ...
                                    fill: false,
                                    tension: 0.1,
                                    pointRadius: 0 // Hide points on the line for clarity
                                },
                                {
                                    label: 'Current Progress',
                                    data: [], // Will be populated dynamically
                                    pointRadius: 6, // Larger dot for visibility
                                    pointBackgroundColor: 'blue', // Blue dot
                                    pointBorderColor: 'blue',
                                    pointHoverRadius: 8,
                                    showLine: false // No line, just the dot
                                }
                            ]
                        },
                        options: {
                                ...
                                tooltip: {
                                    callbacks: {
                                        label: function(context) {
                                            return `${context.dataset.label}: ${context.raw.y} kg (Week ${context.label})`;
                                        }
                                    }
                                }
                            }
                    ...
                    // Calculate progress and update the chart
                    function updateProgressDot(data) {
                        if (!data.started_tracking || !data.projectedWeightTrend || data.projectedWeightTrend.length === 0) {
                            console.warn("No started_tracking or projectedWeightTrend data available.");
                            return;
                        }

                        const startDate = new Date(data.started_tracking);
                        const currentDate = new Date(); // Use Date object, not string
                        const timeDiff = currentDate - startDate; // Difference in milliseconds
                        const weeksElapsed = Math.floor(timeDiff / (1000 * 60 * 60 * 24 * 7)); // Convert to weeks

                        // Ensure weeksElapsed is within the chart range (0 to estimatedWeeksToGoal)
                        const weekIndex = Math.min(Math.max(weeksElapsed, 0), data.estimatedWeeksToGoal);
                        const progressWeight = data.projectedWeightTrend[weekIndex];

                        // Update the progress dataset
                        window.weightChart.data.datasets[1].data = [{ x: `Week ${weekIndex}`, y: progressWeight }];
                        window.weightChart.update();
                    }

                    // Call the function after chart creation
                    updateProgressDot(data);
                    } else {
    ...
    document.getElementById('startTrackingBtn').addEventListener('click', async function() {
        console.log('Tracking started on', new Date().toLocaleString());
        const userId = localStorage.getItem("userId");
        await fetch(`http://localhost:3000/api/fitness/updateFitnessDataTracking`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ userId, started_tracking: new Date() })
        });
    ...
    bodyTypeCards.forEach(card => {
        ...
        if (this.classList.contains('selected')) {
            ...
            if (predictionResponse.ok) {
                ...
                // Adjust target weight based on target body type
                const targetBMI = {
                    'Ectomorph': 19,  // Lower BMI for leaner frame
                    'Mesomorph': 22,  // Balanced BMI
                    'Endomorph': 24   // Slightly higher BMI for stockier build
                }[selectedBodyType] || 22; // Default to 22 if undefined
                targetWeight = Math.round(targetBMI * ((height / 100) ** 2));

                // Ensure target weight is not higher than current weight for reduction
                if (selectedBodyType === 'Ectomorph' && targetWeight > weight) {
                    targetWeight = Math.max(Math.round(18.5 * ((height / 100) ** 2)), weight - 5); // Minimum healthy weight or 5 kg less
                }

                // Calculate calories based on weight change goal
                const totalWeightChange = targetWeight - weight;
                caloriesPerDay = totalWeightChange > 0 ? tdee + 250 : totalWeightChange < 0 ? tdee - 500 : tdee;

                // Calculate nutritional intake
                proteinIntake = Math.round((caloriesPerDay * 0.35) / 4);
                fatIntake = Math.round((caloriesPerDay * 0.35) / 9);
                carbIntake = Math.round((caloriesPerDay * 0.30) / 4);

                estimatedWeeksToGoal = Math.round(estimatedDuration / 7);
                const weeklyWeightChange = totalWeightChange / estimatedWeeksToGoal;
                projectedWeightTrend = Array.from(
                    { length: estimatedWeeksToGoal + 1 },
                    (_, i) => Math.round((weight + weeklyWeightChange * i) * 10) / 10
                );
